...
export default function PricingRow({
  ...
  description,
  onDataChange,
}: PricingType & { onDataChange?: () => void }) {
  const [open, setOpen] = React.useState(false)
  const router = useRouter()

  const handleOpen = () => setOpen((prev) => !prev)

  const handleDeletePricing = useDeleteResource({
  ...
  const handleUpdatePricing = () => {
    ...
    apiFetch(`/pricing/${ID}`, {
      ...
      .then((data) => {
        console.log('Pricing information updated successfully:', data)
        setOpen(false)
        Swal.fire({
      ...
      .catch((error) => {
        ...
        if (error.message !== 'Unauthorized') {
          Swal.fire({
            text: 'Gagal memperbarui data harga.',
            icon: 'error',
            confirmButtonText: 'OK',
          })
        }
